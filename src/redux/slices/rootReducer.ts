...
import { DBPizzasWithStatus } from './type';

export const initialState: DBPizzasWithStatus = {
  pizza: [
    // {
    //   id: 0,
    //   imageUrl:
...
const rootSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(thunks.getAllPizzas.fulfilled, (state, action) => {
      ...
      .addCase(thunks.getSortPizzas.fulfilled, (state, action) => {
        state.pizza = action.payload;
        state.isLoading = false;
      })
      .addCase(thunks.getSortPizzas.pending, (state) => {
        state.isError = '';
        state.isLoading = true;
      })
      .addCase(thunks.getSortPizzas.rejected, (state) => {
        state.isError = 'Error from getAllPizzas';
        state.isLoading = false;
      });
  },
